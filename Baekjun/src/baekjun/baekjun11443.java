...
public class baekjun11443 {
    ...
    static void solution() {
        ...
        // f(2n) = f(2n + 1) - f(2n - 1)
        // 위 식들을 모두 더해보면
        // sum(f(2n)) = f(2n + 1) - f(1) = f(2n + 1) - 1
        // 즉, f(n + 1)을 구한 후에 1을 빼주면 원하는 정답을 얻을 수 있다
        // 2. n이 홀수라면
        // sum(f(2n - 1)) = f((2n - 1) + 1) - f(1) = f(2n) - 1
        // 즉, f(n)을 구한 후에 1을 빼주면 원하는 정답을 얻을 수 있다
        int fibo = findFibo(n);
