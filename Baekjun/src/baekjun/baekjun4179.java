...
public class baekjun4179 {
	...
	static void solution() {
		times = new int[R][C];
		for(int row = 0; row < R; row++) Arrays.fill(times[row], Integer.MAX_VALUE);
		times[start[0]][start[1]] = 0;
		fireTimes = new int[R][C];
		Queue<int[]> loc = new LinkedList<>();
		loc.offer(start);
		int time = 0;
		while(!loc.isEmpty()) {
			int[] cur = loc.poll();
			if(time == times[cur[0]][cur[1]]) {
				spreadFire();
				time++;
			}
	...
	static void spreadFire() {
		int time = times[fires.get(0)[0]][fires.get(0)[1]], size = fires.size();
		Queue<int[]> fireLoc = new LinkedList<int[]>();
		for(int index = 0; index < size; index++) {
			fireLoc.offer(fires.get(index));
			fireLoc.remove(0);
		}
		while(!fireLoc.isEmpty()) {
			int[] cur = fireLoc.poll();
			if(fireTimes[cur[0]][cur[1]] > time) return;
			for(int dir = 0; dir < 4; dir++) {
				int cx = cur[0] + dx[dir], cy = cur[1] + dy[dir];
				if(isInMap(cx, cy)) {
					if(map[cx][cy] == '.') {
						...
						fireTimes[cx][cy] = fireTimes[cur[0]][cur[1]] + 1;
					}
				}
			}
		}
